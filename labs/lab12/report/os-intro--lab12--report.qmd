...
## Lab 12 Additional Tasks

### Task 1: Self-backup script

Created a script that backs itself up to a backup directory in the home folder. The script creates a zip archive with a timestamp in the filename.
...
### Task 2: Script to handle arbitrary number of arguments

Created a script that can handle any number of arguments, including more than ten. It demonstrates two methods of argument processing.
...
### Task 3: ls-like script without using ls or dir

Created a script that mimics the functionality of the `ls` command without using `ls` or `dir`. It displays file information including permissions, owner, size, and modification time.
...
### Task 4: Count files by extension in directory

Created a script that counts files with a specific extension in a specified directory.
